...
export default function Card() {
    ...
    const { searchText, type, capacity, sortby } = useSearch();
    ...
    const filteredVehicles = vehicles
        .filter(vehicle => {
            const matchesSearch = vehicle.vehicleName.toLowerCase().includes(searchText.toLowerCase());
            const matchesType = type ? vehicle.category === type : true;
            const matchesCapacity = capacity ? vehicle.capacity === parseInt(capacity) : true;
            return matchesSearch && matchesType && matchesCapacity;
        })
        .sort((a, b) => {
            if (sortby === 'low-to-high') return a.offerPrice - b.offerPrice;
            if (sortby === 'high-to-low') return b.offerPrice - a.offerPrice;
            return 0;
        });
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">

                    {
                        filteredVehicles.map((vehicle, index) => (
                            <div key={index} className="bg-white border my-5 border-gray-200 rounded-lg shadow px-5 py-5 flex flex-col justify-between">
                                ...
                                >
                                    <img
                                        className="p-5 object-contain h-full w-full rounded-t-lg"
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Link state={{ vehicle }} to="/booking" className="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded text-xs px-3 py-2 text-center ">Rent Now</Link>
                                    </div>
                                </div>
                            </div>
                        ))
